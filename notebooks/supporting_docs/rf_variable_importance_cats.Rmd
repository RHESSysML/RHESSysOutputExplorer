...
df_wy_num_preds <- df_wy_cats %>% 
  select(!response & where(is.numeric))

df_wy_fpreds <- df_wy_cats %>% 
...
imp <- train(x = df_wy_cats %>% select(!response),
              y = df_wy_cats$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
...
df_pred_binned <- create_binned_df(df_wy_cats)
```
