...
imp_clim0 <- varImp(rf_clim0$finalModel, scale = FALSE) %>% 
  rownames_to_column("Variable") %>% 
  mutate(Rank = rank(-Overall)) %>% 
  arrange(Rank)

imp_clim2 <- varImp(rf_clim2$finalModel, scale = FALSE) %>% 
  rownames_to_column("Variable") %>% 
  mutate(Rank = rank(-Overall)) %>% 
  arrange(Rank)
```
...
wy0_plot <- plot_imp(imp_clim0)
wy2_plot <- plot_imp(imp_clim2)
...
```{r save_model_output}
save(rf_clim0, rf_clim2, imp_clim0, imp_clim2, file = here::here("data", "output", "model_output.RData"))
```
