...
summarize_var_removal(df_clim0_num_preds, table = TRUE)
```
...
summarize_var_removal(df_clim2_num_preds, table = TRUE)
```
...
rf_clim0 <- train(x = df_clim0_reduced %>% select(!response),
...
rf_clim2 <- train(x = df_clim2_reduced %>% select(!response),
...
saveRDS(rf_clim0, here::here("data", "rf_clim0.RDS"))
saveRDS(rf_clim2, here::here("data", "rf_clim2.RDS"))
```
...
imp_wy0 <- varImp(rf_clim0$finalModel, scale = FALSE) %>% 
  rownames_to_column("Variable") %>% 
  mutate(Rank = rank(-Overall))

imp_wy2 <- varImp(rf_clim2$finalModel, scale = FALSE) %>% 
  rownames_to_column("Variable") %>% 
  mutate(Rank = rank(-Overall))
```
...
```{r}
rf_clim0_fit <- round(tail(rf_clim0$finalModel$rsq, 1)*100, 2)
...
```{r}
rf_clim2_fit <- round(tail(rf_clim2$finalModel$rsq, 1)*100, 2)
