...
```{r}
gb_fit0 <- round(tail(gbm_model_cv0_best$results$Rsquared, 1)*100, 2)

gb_fit2 <- round(tail(gbm_model_cv2_best$results$Rsquared, 1)*100, 2)
...
df_imp_table(imp_gb0, imp_gb2)
```

```{r plot_imp}
source(here::here("R", "plot_imp.R"))

clim0_plot <- plot_imp(imp_gb0)
clim2_plot <- plot_imp(imp_gb2)
...
```{r}
gbm_model_cv0_best$results$RMSE
gbm_model_cv2_best$results$Rsquared
...
Run the following code chunk to launch the Shiny app:
