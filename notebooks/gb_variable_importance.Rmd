...
# Find preliminary importance using random forest
imp2 <- train(x = df_clim2 %>% select(!response),
              y = df_clim2$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
...
wy0_vif <- remove_vif(df_clim0_num_preds, vif.threshold = vif.threshold, pref_order0)$selected.variables
wy0_cor <- remove_cor(df_clim0_num_preds, cor.threshold = cor.threshold, pref_order0)$selected.variables
wy0_select_variables <- intersect(wy0_vif, wy0_cor)

wy2_vif <- remove_vif(df_clim2_num_preds, vif.threshold = vif.threshold, pref_order2)$selected.variables
wy2_cor <- remove_cor(df_clim2_num_preds, cor.threshold = cor.threshold, pref_order2)$selected.variables
wy2_select_variables <- intersect(wy2_vif, wy2_cor)

# Combine selected variables with factors
df_clim0_preds <- c(all_of(df_clim0_fpreds), all_of(wy0_select_variables))
df_clim2_preds <- c(all_of(df_clim2_fpreds), all_of(wy2_select_variables))
...
summarize_removed_vars(reduced_df = df_clim0_num_preds, table = TRUE)
```

#### Correlation Summary Table

```{r cor_summary_table_0}
summarize_cor(reduced_df = df_clim0_num_preds, selected_removed = "removed")
```

#### Importance Summary Plot

```{r imp_summary_plot_0, warning=FALSE}
plot_removed_imp(df_clim0_num_preds)
```

#### VIF Summary Plot

```{r vif_summary_plot_0, warning=FALSE}
plot_removed_vif(df_clim0_num_preds)
```
...
summarize_removed_vars(df_clim2_num_preds, table = TRUE)
```

#### Correlation Summary Table

```{r cor_summary_table_2}
summarize_cor(df_clim2_num_preds, selected_removed = "removed")
```

#### Importance Summary Plot

```{r imp_summary_plot_2, warning=FALSE}
plot_removed_imp(df_clim2_num_preds)
```

#### VIF Summary Plot

```{r vif_summary_plot_2, warning=FALSE}
plot_removed_vif(df_clim2_num_preds)
```
...
imp_gb0 = gbm_imp0$importance
imp_gb0 = imp_gb0 %>% mutate(Variable = row.names(imp_gb0)) %>% 
  filter(imp_gb0$Overall != 0)

gbm_imp2 = varImp(gbm_model_cv2_best, scale = FALSE)
imp_gb2 = gbm_imp2$importance
imp_gb2 = imp_gb2 %>% mutate(Variable = row.names(imp_gb2)) %>% 
  filter(imp_gb2$Overall != 0)
...
# saveRDS(imp_gb0, file = here::here("shiny", "aggregated_datasets", "imp_gb0.RDS"))
# saveRDS(imp_gb2, file = here::here("shiny", "aggregated_datasets", "imp_gb2.RDS"))
```
