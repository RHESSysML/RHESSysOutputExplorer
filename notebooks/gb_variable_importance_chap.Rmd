...
gbm_imp = varImp(model, scale = FALSE)
imp = gbm_imp$importance
imp = imp %>% mutate(Variable = row.names(imp)) %>% 
  filter(imp$Overall != 0)
...
```{r}
gb_fit <- round(tail(model$results$Rsquared, 1)*100, 2)
...
```{r save_model_output}
save(model, imp, file = here::here("data", "output", "model_output_chap.RData"))
```
