---
title: "Variable Importance Workflow, Chaparral - Gradient Boosting"
...
no_var <- sapply(df_num_preds, function(col) var(col) == 0)
df_num_preds <- df_num_preds[, !no_var]
...
summarize_var_removal(df_num_preds, table = TRUE)
```
...
```{r}
# In the case of this dataset, there are no factor variables remaining so there is no need to one-hot encode anything.
...
gbm_model_cv <- read_rds(here::here("data", "input", "GBm_chap.RData"))
```

To get the model with the best-performing parameter set from the tuning grid above, we simply select the `bestTune` element from our model. Running this model individually will then allow us to extract accuracy and importance metrics more easily in the following sections.
...
```{r save_model_output}
save(imp_gb, file = here::here("data", "output", "model_output_chap.RData"))
```
...
if (run_app == TRUE) {
  shiny::runApp(here::here("shiny_chap"))
}
```
