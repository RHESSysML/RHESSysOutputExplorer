...
df_raw <- readRDS(here::here("data", "sageres.RDS")) 
```
...
```{r}
# For the final product, this will save the aggregated datasets as csv files that will get overwritten if the user changes the way they prepared their data, or if they use a new dataset. While we are working on this project we will comment this code out so that new files don't get written everytime. If the user wants to save older versions of their aggregated data, they just need to give it a unique name every time they run this code.
...
df_export <- rename(.data = df, !!response_var := response)
df_export_clim0 <- rename(.data = df_clim0, !!response_var := response)
df_export_clim2 <- rename(.data = df_clim2, !!response_var := response)

# write.csv(df_export, file = here("shiny", "aggregated_datasets", "df_wy.csv"), row.names = FALSE)
# write.csv(df_export_clim0, file = here("shiny", "aggregated_datasets", "df_clim0.csv"), row.names = FALSE)
# write.csv(df_export_clim2, file = here("shiny", "aggregated_datasets", "df_clim2.csv"), row.names = FALSE)

saveRDS(df_export, file = here::here("shiny", "aggregated_datasets", "df.RDS"))
saveRDS(df_export_clim0, file = here::here("shiny", "aggregated_datasets", "df_clim0.RDS"))
saveRDS(df_export_clim2, file = here::here("shiny", "aggregated_datasets", "df_clim2.RDS"))
```

```{r}
save.image(here::here("data", "prepared_data.RData"))
```
