...
summarize_var_removal(df_wy_num_preds, table = TRUE)
```

#### Cor Summary Plot

```{r}
plot_removed_cor(wy_select_variables, df_wy_num_preds)
```
...
Variable importance plots display relative magnitude of importance between variables. The plot below shows that for the response variable `plantc` (total plant carbon), the top 3 most important predictive variables are `height`, `resp`, and `Kstar_soil`. The relationship between height and total carbon is unsurprising (taller trees contain more physical matter, therefore more carbon); likewise, it makes sense that higher respiration rates correlate with more carbon. But the `Kstar_soil` variable perhaps provides a  place to ask more interesting questions - what is it about shortwave radiation absorbed by soil that has such an effect on plant carbon?
...
While permutation importance was chosen as the primary measure of importance in this workflow, there are other measures that random forests allow for which may be of utility. Although, it's important that the inherent biases previously mentioned (Strobl et al. 2007) are understood. These additional measures will not be described further here, but the function below, `randomForestExplainer::explain_forest()`, is an excellent tool which creates an html document summarizing and comparing a number of them.
...
if (run_explainer == TRUE) {
  library(randomForestExplainer)
  randomForestExplainer::explain_forest(rf_wy0, path = here::here("docs", "forest_explained.html"))
}
```
